...
Usage:
  1. Set your Gemini API key below (or as env var GEMINI_API_KEY)
...
# ──────────────────────────────────────────────
#  Run AI analysis via Gemini API (FREE)
# ──────────────────────────────────────────────
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/" or self.path == "/index.html":
            ...
            self.send_json(200, {"status": "ok", "apiKeySet": GEMINI_API_KEY != "YOUR_GEMINI_KEY_HERE"})
        else:
        ...
        if self.path == "/api/scan":
            try:
                ...
                if GEMINI_API_KEY == "YOUR_GEMINI_KEY_HERE":
                    self.send_json(400, {"error": "Gemini API key not set. Edit polyedge_server.py and set GEMINI_API_KEY."})
                    return
    ...
    if (!d.apiKeySet) {
      document.getElementById('content').innerHTML = `
        <div class="api-warning">
          ⚠ <strong>Gemini API key not configured.</strong><br><br>
          Open <code>polyedge_server.py</code> and replace <code>YOUR_GEMINI_KEY_HERE</code> with your free Gemini API key.<br><br>
          Get your FREE key at <strong>aistudio.google.com</strong> → Get API Key (takes 30 seconds)
        </div>
...
║                                              ║
║  1. Get FREE key: aistudio.google.com        ║
║  2. Set GEMINI_API_KEY in this file          ║
║     OR run:                                  ║
║     GEMINI_API_KEY=AIza... python \\          ║
║       polyedge_server.py                     ║
║                                              ║
